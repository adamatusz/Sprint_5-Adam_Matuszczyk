...
[
  ...
  const newArray = persons.map((person) => {
    if (typeof person.firstName === "string" &&
        ...
        const arrayFromPersonFirstName = person.firstName.match(/./ug); // musiałem regexem strasznie śmieszne
        const arrayFromPersonLastName = person.lastName.match(/./ug);
        const nickName = [].concat(
          arrayFromPersonFirstName[arrayFromPersonFirstName.length - 1].toUpperCase(),
          arrayFromPersonFirstName[arrayFromPersonFirstName.length - 2],
          arrayFromPersonFirstName[arrayFromPersonFirstName.length - 3].toLowerCase(),
